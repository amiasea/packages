...
1. Overview  
Renderer Adapters form a newly defined subsystem category within the AUI pipeline.  
They act as deterministic, semantics‑preserving compilers that transform **validated, canonical AUI** into **platform‑specific render artifacts**.

This subsystem exists only because of the strict architectural boundaries established in:  
- **RFC‑003 — Semantic Validator**, which guarantees the canonical input format  
- **RFC‑005 — AUI Toolchain**, which orchestrates pipeline ordering and artifact flow  

Renderer Adapters sit precisely between:  
- **Semantic Layer** (output of RFC‑003)  
- **Platform Layer** (input to the Host Integrator)  

Their purpose is to **map semantics to platform primitives without altering meaning, structure, or intent**.

Renderer Adapters MUST:  
- map semantic kinds → platform primitives  
- map semantic props → platform props  
- expand composite components deterministically  
- resolve semantic parts  
- prepare symbolic events for downstream interpretation  
- preserve structural and semantic equivalence  
...
2.1 Why This Category Is New  
Traditional UI systems provide:  
- renderers  
- compilers  
- templating engines  
- component frameworks  

None of these operate on a **semantic IR** or enforce **semantic preservation** as a first‑class invariant.

A Renderer Adapter is new because it:  
- consumes **canonical AUI**, not templates or code  
- performs **semantic‑to‑platform mapping**, not rendering  
- is **deterministic**, not reactive  
- is **pure**, not tied to runtime state  
- is **contract‑driven**, not heuristic  
- is **renderer‑agnostic**, not bound to a UI library  

This subsystem exists to uphold the AUI pipeline’s strict separation of:  
- intent  
- semantics  
- validation (RFC‑003)  
- mapping (this RFC)  
- orchestration (RFC‑005)  
- rendering  
- platform integration  
...
| System | Category | Relationship | Reference |
| --- | --- | --- | --- |
| Semantic Validator | Validation Engine | Produces the canonical, normalized AUI artifact the adapter consumes. | RFC‑003 |
| AUI Toolchain | Orchestration | Ensures the adapter runs after validation and before rendering; manages artifact flow. | RFC‑005 |
| Component Contracts | Contract Layer | Defines allowed kinds/props/parts the adapter must map. | RFC‑006 |
| Semantic Event Model | Interaction Semantics | Defines symbolic event semantics preserved by the adapter. | RFC‑007 |
| Data Model & Binding | Data Semantics | Resolves bindings before adapter hydration; adapter must not evaluate bindings. | RFC‑008 |
| Rendering Semantics | Renderer Semantics | Defines downstream interpretation rules the adapter must satisfy. | RFC‑012 |

Renderer Adapters do not replace any of these tools.  
They form a **new, required, non‑optional stage** in the AUI pipeline.
...
Renderer Adapters MUST accept only:  
- **artifact.aui**  
- produced by the Semantic Validator (RFC‑003)  
- canonical  
- normalized  
- semantically correct  
...
Renderer Adapters consist of four deterministic subsystems:  
- **Kind Mapper**  
- **Prop Mapper**  
- **Component Expander**  
- **Event Preparer**  
...
7. Kind Mapper (Subsystem)  
Category: Semantic‑to‑Platform Mapping Engine  
Novelty: First mapping layer that treats UI semantics as a stable IR.
...
8. Prop Mapper (Subsystem)  
Category: Semantic‑to‑Platform Mapping Engine  
Novelty: Props are semantic, not renderer‑specific.
...
9. Component Expander (Subsystem)  
Category: Semantic‑to‑Platform Mapping Engine  
Novelty: Expansion is deterministic and contract‑driven.
