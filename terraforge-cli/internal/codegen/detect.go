...
// DetectFromSchema inspects the raw generator model and determines
...
func DetectFromIR(g *irgraph.Graph) Kind {
	if len(g.Nodes) == 0 {
		return KindUnknown
	}

	// If the IR graph contains edges, it’s likely a diagram or module run.
	if len(g.Edges) > 0 {
		return KindDiagram
	}

	// If the IR graph contains nodes but no edges, it’s likely a module or provider run.
	return KindModule
}
